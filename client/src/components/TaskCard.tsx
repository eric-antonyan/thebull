...
const TaskCard: FC<ITaskCard> = ({ task }) => (
  <Link
    to={`/tasks/${task._id}`}
    className={`relative bg-gray-800 cursor-pointer p-4 flex flex-col rounded-3xl w-full overflow-hidden shadow-lg transition-transform transform`}
  >
    <h3 className="font-bold text-white text-lg">
      {task.title && task.title.slice(0, 25)}
